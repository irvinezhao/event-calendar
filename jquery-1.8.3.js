...
(function(window, undefined) {
	...
	jQuery.event = {
		...
		trigger: function(event, data, elem, onlyHandlers) {
			...
			event =
				typeof event === "object" // jQuery.Event object
				?
				event[jQuery.expando] ?
				event // Object literal
				:
		...
		function select(selector, context, results, seed, xml) {
			...
			if (!seed) {
				...
				if (match.length === 1) {
					...
					for (
						i = matchExpr["POS"].test(selector) ? -1 : tokens.length - 1; i >= 0; i--
					) {
						token = tokens[i];
	...
	jQuery.fn.load = function(url, params, callback) {
		...
		jQuery
			...
			.done(function(responseText) {
				...
				self.html(
					selector // Create a dummy div to hold the results
					?
					jQuery("<div>")
